...
import { REQUESTOR_NOTIFICATIONS, type NotifIcon } from "@/data/notifications";
...
export default function RequestorNotificationsPage() {
  return (
    <div style={{ maxWidth: 640, margin: "0 auto" }}>
      ...
      <GlassCard>
        <CardHeader>
          <span style={{ fontSize: 13, color: "var(--text-muted)" }}>{REQUESTOR_NOTIFICATIONS.length} notifications</span>
          <button className="btn btn-secondary btn-sm">Mark all read</button>
        </CardHeader>
        <CardBody style={{ padding: 0 }}>
          {REQUESTOR_NOTIFICATIONS.map((n, i) => {
            const Icon = ICON_MAP[n.icon] ?? Info;
            const color = COLOR_MAP[n.iconColor] ?? "var(--gold)";
            return (
              <div key={n.id} style={{
                display: "flex", gap: 12, padding: "14px 18px",
                borderBottom: i < REQUESTOR_NOTIFICATIONS.length - 1 ? "1px solid var(--border)" : "none",
